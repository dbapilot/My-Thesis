...
\begin{itemize}
    ...
    Materialized views require periodic refreshing to maintain consistency with the underlying data. This research employs the on-demand refresh mechanism, one of several available strategies. The on-demand refresh approach updates the materialized view in response to specific events or user requests. The on-demand refresh strategy is selected for this study due to its flexibility and resource efficiency, allowing for targeted updates when data accuracy is most critical.
