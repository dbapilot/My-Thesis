...
\subsubsection{Challenges in Query Optimization} Query optimization is a crucial aspect of database management, aiming to improve the performance and efficiency of SQL queries. Opta data gruppe, a company that specializing in healthcare data management, faces several challenges in query optimization within its MSSQL. Given the vast amount of patient and operational data they handle, inefficient queries can lead to slow response times and increased resource consumption \cite{Flipico-2024}. The following example scenario shows query challenges:\vspace{.4cm}
...
Two main equations are involved in the process of PSO algorithm:
\begin{itemize}
