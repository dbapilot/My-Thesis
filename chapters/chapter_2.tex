...
This chapter describes the most significant parts and fully grasps the fundamental ideas and principles that make query optimization and materialized views effective. The respective sections of the thesis explain the concrete basics needed.
...
The result of a query is generated by processing the rows and columns in a database in a way that yields the requested information. Since database structures are complex, in most cases, and especially for not very simple queries, the needed data for a query can be collected from a database by accessing it in different ways through different data structures and in different orders \cite{selinger-1979}. Each different way typically requires a different processing time. The processing time of the same query may have a large variance, from a fraction of a second to hours, depending on the chosen method. The purpose of query optimization is to find a way to process a given query in a minimum time. The large possible variance in time justifies performing query optimization. However, finding the exact optimal query plan among all possibilities is typically very complex, time-consuming by itself may be too costly, and often practically impossible. Thus, query optimization typically tries to approximate the optimum by comparing several common-sense alternatives to provide in a reasonable time.\vspace{.4cm}
...
The motivation for using materialized views is to improve performance, but the extra effort associated with materialized view management can become a significant system management concern. The common materialized view management activities include identifying which materialized view to generate, indexing the view, and ensuring that all materialized views and indexes are refreshed properly each time the database is updated. Examining which materialized views have been used and determining how effective each materialized view has been on workload performance, which comprises the common materialized view management tasks. Besides that, measuring the space being used by materialized views and deciding which existing materialized views should be dropped, archiving old detail and materialized view data that is no longer useful as well \cite{Ashadevi2008CostEA,1363763}.

\subsubsection{Challenges in Query Optimization} Query optimization is a crucial aspect of database management, aiming to improve the performance and efficiency of SQL queries. Opta data Group, a company that specialices in healthcare data management, faces several challenges in query optimization within its MSSQL. Given the vast amount of patient and operational data they handle, inefficient queries can lead to slow response times and increased resource consumption \cite{Flipico-2024}. The following example scenario shows query challenges:\vspace{.4cm}
