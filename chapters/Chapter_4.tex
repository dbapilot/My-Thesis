...
\begin{tikzpicture}
    ...
    \addplot[
        fill=blue!30, % Bar fill color
        draw=blue, % Bar border color
    ] coordinates {(Before MV, 85) (After MV, 30)}; % CPU Usage values
    \legend{Avg CPU Usage per Query}
    \end{axis}
\end{tikzpicture}
\caption{Comparison of CPU Usage Before and After Implementing Materialized Views}
\label{fig:cpu_usage}
\end{center}
\end{figure}
\end{enumerate}
