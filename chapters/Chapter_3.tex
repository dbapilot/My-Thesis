...
This thesis primarily explores the use of materialized views and the PSO algorithm to improve query performance. It highlights the connection between computing costs and storage costs. Computing costs, which are affected by workload and data size, play a vital role in assessing the PSO algorithm's effectiveness in optimizing query execution plans. In contrast, storage costs are also significant, as materialized views require additional disk space, especially in large databases. The necessary trade-off between quicker query execution and increased storage needs is essential. By examining these factors, the thesis aims to illustrate how PSO can shift the balance toward enhanced overall query execution by weighing the advantages and disadvantages of materialized views.
